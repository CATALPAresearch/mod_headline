...
/**
 * Library of functions and constants for module headline
 *
...
/** headline_MAX_NAME_LENGTH = 50 */
define("HEADLINE_MAX_NAME_LENGTH", 50);

/**
 * @uses HEADLINE_MAX_NAME_LENGTH
 * @param object $headline
 * @return string
 */
function get_headline_name($headline) {
    $name = strip_tags(format_string($headline->intro,true));
    if (core_text::strlen($name) > HEADLINE_MAX_NAME_LENGTH) {
        $name = core_text::substr($name, 0, HEADLINE_MAX_NAME_LENGTH)."...";
    }

    if (empty($name)) {
        // arbitrary name
        $name = get_string('modulename','headline');
    }
 ...
 * @return bool|int
 */
function headline_add_instance($headline) {
    global $DB;

    $headline->name = get_headline_name($headline);
    $headline->timemodified = time();

    $id = $DB->insert_record("headline", $headline);

    $completiontimeexpected = !empty($headline->completionexpected) ? $headline->completionexpected : null;
    \core_completion\api::update_completion_date_event($headline->coursemodule, 'headline', $id, $completiontimeexpected);
 ...
 * @return bool
 */
function headline_update_instance($headline) {
    global $DB;

    $headline->name = get_headline_name($headline);
    $headline->timemodified = time();
    $headline->id = $headline->instance;

    $completiontimeexpected = !empty($headline->completionexpected) ? $headline->completionexpected : null;
    \core_completion\api::update_completion_date_event($headline->coursemodule, 'headline', $headline->id, $completiontimeexpected);

    return $DB->update_record("headline", $headline);
}
 ...
 * @return bool
 */
function headline_delete_instance($id) {
    global $DB;

    if (! $headline = $DB->get_record("headline", array("id"=>$id))) {
        return false;
    }

    $result = true;

    $cm = get_coursemodule_from_instance('headline', $id);
    \core_completion\api::update_completion_date_event($cm->id, 'headline', $headline->id, null);

    if (! $DB->delete_records("headline", array("id"=>$headline->id))) {
        $result = false;
    }
 ...
 */
function headline_get_coursemodule_info($coursemodule) {
    global $DB;

    if ($headline = $DB->get_record('headline', array('id'=>$coursemodule->instance), 'id, name, intro, introformat')) {
        if (empty($headline->name)) {
            // headline name missing, fix it
            $headline->name = "label{$headline->id}";
            $DB->set_field('headline', 'name', $headline->name, array('id'=>$headline->id));
        }
        $info = new cached_cm_info();
        // no filtering hre because this info is cached and filtered later
        $info->content = format_module_intro('headline', $headline, $coursemodule->id, false);
        $info->name  = $headline->name;
        return $info;
    } else {
        return null;
    }
 ...
 */
function headline_reset_userdata($data) {
 ...
 */
function headline_supports($feature) {
    switch($feature) {
 ...
 */
function headline_dndupload_register() {
    $strdnd = get_string('dnduploadlabel', 'mod_label');
    if (get_config('headline', 'dndmedia')) {
 ...
 */
function headline_dndupload_handle($uploadinfo) {
    global $USER;
    ...
    if (!empty($uploadinfo->draftitemid)) {
        ...
        if ($file = reset($files)) {
            if (file_mimetype_in_typegroup($file->get_mimetype(), 'web_image')) {
                ...
                $config = get_config('headline');
                $data->intro = headline_generate_resized_image($file, $config->dndresizewidth, $config->dndresizeheight);
            } else {
    ...
    return headline_add_instance($data, null);
}
 ...
 */
function headline_generate_resized_image(stored_file $file, $maxwidth, $maxheight) {
    global $CFG;
 ...
 * @since Moodle 3.2
 */
function headline_check_updates_since(cm_info $cm, $from, $filter = array()) {
 ...
 */
function mod_headline_core_calendar_provide_event_action(calendar_event $event,
                                                      \core_calendar\action_factory $factory,
                                                      int $userid = 0) {
    $cm = get_fast_modinfo($event->courseid, $userid)->instances['headline'][$event->instance];
